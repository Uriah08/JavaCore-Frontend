...
import { useDeleteJobsMutation } from "@/store/job-api"
      ...
      <div className="flex xl:flex-row flex-col gap-3 justify-between">
        ...
        <div className="flex flex-1 gap-2 items-center w-full">
          ...
          <PopoverTrigger asChild>
            ...
            >
              <CalendarIcon />
              {fromDate ? format(fromDate, "PPP") : <span className='text-xs sm:text-sm'>Pick a date</span>}
            </Button>
          </PopoverTrigger>
        ...
        <div className="flex flex-1 gap-2 items-center">
          ...
          <PopoverTrigger asChild>
            ...
            >
              <CalendarIcon />
              {toDate ? format(toDate, "PPP") : <span className='text-xs sm:text-sm'>Pick a date</span>}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0" align="start">
            <Calendar
      ...
      <div className="flex items-center py-4 w-full justify-between gap-3">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger disabled={deleteLoading || Object.keys(rowSelection).length === 0}>
            <div className={`bg-main rounded-md p-2 text-white ${deleteLoading || Object.keys(rowSelection).length === 0 ? 'opacity-50': 'hover:bg-follow cursor-pointer'} ${authUser?.role === 'user' && 'hidden'}`}>
              <Trash size={20}/>
            </div>
          </DialogTrigger>
          <DialogContent aria-describedby={undefined}>
            ...
            <div className="flex gap-3 justify-end">
              <DialogClose asChild>
                <Button className="cursor-pointer" onClick={() => setRowSelection({})} variant={"outline"}>Cancel</Button>
              </DialogClose>
              <Button className="bg-main hover:bg-follow cursor-pointer" onClick={deleteJob} disabled={deleteLoading || Object.keys(rowSelection).length === 0}>Delete</Button>
            </div>
          </DialogContent>
        </Dialog>
        <DropdownMenu>
